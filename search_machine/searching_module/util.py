...
def merge_and_sort_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    pointer_1 = 0
    pointer_2 = 0
    while pointer_1 < len(posts_tfs1) or pointer_2 < len(posts_tfs2):

        if pointer_1 == len(posts_tfs1):
            result.append(posts_tfs2[pointer_2])
            pointer_2 += 1

        elif pointer_2 == len(posts_tfs2):
            result.append(posts_tfs1[pointer_1])
            pointer_1 += 1
        
        elif posts_tfs1[pointer_1][0] < posts_tfs2[pointer_2][0]:
            result.append(posts_tfs1[pointer_1])
            pointer_1 += 1
        
        elif posts_tfs2[pointer_2][0] < posts_tfs1[pointer_1][0]:
            result.append(posts_tfs2[pointer_2])
            pointer_2 += 1
        
        else:
            total_freq = posts_tfs1[pointer_1][1] + posts_tfs2[pointer_2][1]
            result.append((posts_tfs1[pointer_1][0], total_freq))
            pointer_1 += 1
            pointer_2 += 1
