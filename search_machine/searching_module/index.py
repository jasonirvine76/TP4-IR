...
class InvertedIndex:
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            self.postings_dict, self.terms, self.doc_length, self.avdl, self.term_upper_bound = pickle.load(f)
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        with open(self.metadata_file_path, 'wb') as f:
            pickle.dump([self.postings_dict, self.terms, self.doc_length, self.avdl, self.term_upper_bound], f)
...
class InvertedIndexReader(InvertedIndex):
    ...
    """    
    def load_score(self):
        if(len(self.term_upper_bound) == 0):
            avdl = self.avdl
            k1 = 1.2
            b = 0.75  
            for token_id in self.terms:
                posting_list, tf_list = self.get_postings_list(token_id)
                ...
                idf = math.log10(len(self.doc_length) / len(posting_list))
                for i, doc in enumerate(posting_list):
                    tf = tf_list[i]
                    dl = self.doc_length[doc]
                    bm25 = idf * ((k1 + 1) * tf) / (k1 * ((1 - b) + b * dl / avdl) + tf)
                    maxBM25 = max(maxBM25, bm25)

                    tfidf = idf * (1 + math.log10(tf_list[i]))

                    maxTFIDF = max(maxTFIDF, tfidf)
    ...
    def get_postings_list(self, term):
        ...
        """
        # TODO
        if(self.postings_dict.get(term) == None):
            return []
        start_position, number_of_posting, length_encode, length_encode_tf  = self.postings_dict[term]
        self.index_file.seek(start_position)
        return self.postings_encoding.decode(self.index_file.read(length_encode)), self.postings_encoding.decode_tf(self.index_file.read(length_encode_tf))
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        for i in range(len(postings_list)):
            ...
            if self.doc_length.get(posting) == None:
                self.doc_length[posting] = 0
            self.doc_length[posting] += tf_list[i]
        
        self.index_file.seek(0, 2)
        self.postings_dict[term] = (self.index_file.tell(), len(postings_list), 
                                    len(encoded_posting_list), len(encoded_tf_list))
        self.index_file.write(encoded_posting_list)
        self.index_file.write(encoded_tf_list)

    def __exit__(self, exception_type, exception_value, traceback):
        self.avdl = 0
        sum = 0
        for freq in self.doc_length.values():
            sum += freq
        total_len = len(self.doc_length)
        if(total_len != 0):
            self.avdl = sum / total_len
