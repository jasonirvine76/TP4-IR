...
class VBEPostings:
    ...
    def vb_encode(list_of_numbers):
        ...
        for number in list_of_numbers:
            bytes.extend(VBEPostings.vb_encode_number(number))
    ...
    def encode(postings_list):
        ...
        for i in range(0, len(postings_list)):
            list_doc_id.append(postings_list[i] - current_id)
            current_id = postings_list[i]
        return VBEPostings.vb_encode(list_doc_id)
    ...
    def decode(encoded_postings_list):
        ...
        """
        # TODO
        list_doc_id = VBEPostings.vb_decode(encoded_postings_list)
        encoded_postings_list = []
        current_value = 0
        for i in range(len(list_doc_id)):
            encoded_postings_list.append(current_value + list_doc_id[i])
            current_value += list_doc_id[i]
